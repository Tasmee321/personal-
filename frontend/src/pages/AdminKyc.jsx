...
const AdminKyc = () => {
  ...
  const [dismissingFlag, setDismissingFlag] = useState(null); // flagText being dismissed
  const [trustingWallet, setTrustingWallet] = useState(false);
...
This cannot be undone!`)) return;
  ...
  const loadUserHistory = async (user, tab) => {
    ...
    if (tab === 'transactions') {
      setLoadingHistory(true);
      try {
        const res = await fetch(`${API_URL}/api/admin/user/${user.id}/transactions`, { headers: adminHeaders(adminKey) });
        const data = await res.json();
  ...
  const dismissFlag = async (userId, flag) => {
    setDismissingFlag(flag);
    try {
      const res = await fetch(`${API_URL}/api/admin/user/${userId}/dismiss-flag`, {
        method: 'POST', headers: adminHeaders(adminKey),
        body: JSON.stringify({ flag }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      // Remove flag from selectedUser locally so UI updates immediately
      setSelectedUser(prev => ({ ...prev, flags: (prev.flags || []).filter(f => f !== flag) }));
      setUsers(prev => prev.map(u => u.id === userId ? { ...u, flags: (u.flags || []).filter(f => f !== flag) } : u));
      showToast('Flag dismissed');
    } catch(e) { setError(e.message); }
    finally { setDismissingFlag(null); }
  };

  const trustWallet = async (userId, address) => {
    setTrustingWallet(true);
    try {
      const res = await fetch(`${API_URL}/api/admin/trust-wallet`, {
        method: 'POST', headers: adminHeaders(adminKey),
        body: JSON.stringify({ address }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      // Remove any "Wallet shared" flag from this user
      const walletFlag = (selectedUser.flags || []).find(f => f.startsWith('Wallet shared'));
      if (walletFlag) {
        setSelectedUser(prev => ({ ...prev, flags: (prev.flags || []).filter(f => !f.startsWith('Wallet shared')) }));
        setUsers(prev => prev.map(u => u.id === userId ? { ...u, flags: (u.flags || []).filter(f => !f.startsWith('Wallet shared')) } : u));
      }
      showToast('Wallet marked as trusted');
    } catch(e) { setError(e.message); }
    finally { setTrustingWallet(false); }
  };
                ...
                <tbody>
                  {filteredUsers.slice((usersPage - 1) * USERS_PER_PAGE, usersPage * USERS_PER_PAGE).map(u => (
                    <tr key={u.id} style={{ borderBottom: `1px solid ${theme.cardBorder}`, cursor: 'pointer' }}
                      onClick={() => { setSelectedUser(u); setNewLevel(String(u.level || 0)); setNewLimit(String(u.dailySignalLimit || 3)); setUserDetailTab('info'); setUserDeposits([]); setUserWithdrawals([]); setUserSignals([]); setUserTransactions([]); setTeamTree(null); setTeamData(null); setTeamUser(null); }}>
        ...
        {activeTab === 'users' && selectedUser && (
          <>
              ...
              <div style={card}>
                ...
                {(selectedUser.flags || []).length > 0 && (
                  <div style={{ marginBottom: '16px' }}>
                    <div style={{ fontSize: '11px', fontWeight: '700', color: theme.down, marginBottom: '6px' }}>⚠ Risk Flags</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {selectedUser.flags.map(f => {
                        const isWalletFlag = f.startsWith('Wallet shared');
                        // Extract wallet address from withdrawal requests for trust action
                        const walletAddr = isWalletFlag
                          ? (selectedUser.whitelistedAddresses?.[0] || null)
                          : null;
                        return (
                          <div key={f} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', borderRadius: '8px', backgroundColor: theme.downSoft, border: `1px solid ${theme.down}33` }}>
                            <span style={{ fontSize: '11px', fontWeight: '600', color: theme.down }}>⚠ {f}</span>
                            <div style={{ display: 'flex', gap: '6px' }}>
                              {isWalletFlag && (
                                <button
                                  disabled={trustingWallet}
                                  onClick={() => {
                                    // Get wallet from user's withdrawal requests
                                    const wAddr = selectedUser.whitelistedAddresses?.[0];
                                    if (wAddr) trustWallet(selectedUser.id, wAddr);
                                    else {
                                      // Prompt for wallet address
                                      const addr = prompt('Enter the wallet address to trust:');
                                      if (addr) trustWallet(selectedUser.id, addr);
                                    }
                                  }}
                                  style={{ fontSize: '10px', padding: '3px 8px', borderRadius: '6px', border: 'none', cursor: 'pointer', backgroundColor: '#6366f1', color: '#fff', fontWeight: '600' }}
                                >
                                  {trustingWallet ? '...' : '✓ Trust Wallet'}
                                </button>
                              )}
                              <button
                                disabled={dismissingFlag === f}
                                onClick={() => dismissFlag(selectedUser.id, f)}
                                style={{ fontSize: '10px', padding: '3px 8px', borderRadius: '6px', border: 'none', cursor: 'pointer', backgroundColor: theme.card, color: theme.subtext, fontWeight: '600' }}
                              >
                                {dismissingFlag === f ? '...' : '✕ Dismiss'}
                              </button>
                            </div>
                          </div>
                        );
            ...
            {userDetailTab === 'transactions' && (
              <div style={card}>
                <div style={{ fontWeight: 'bold', fontSize: '14px', marginBottom: '4px' }}>Transaction History</div>
                <div style={{ fontSize: '11px', color: theme.faint, marginBottom: '14px' }}>{userTransactions.length} total entries · all times in PKT</div>
                {loadingHistory && <div style={{ color: theme.faint, textAlign: 'center', padding: '30px' }}>Loading...</div>}
                {!loadingHistory && userTransactions.length === 0 && <div style={{ color: theme.faint, textAlign: 'center', padding: '30px' }}>No transactions found</div>}
                {!loadingHistory && userTransactions.map((tx, i) => {
                  const fmtAmt = (n) => {
                    if (n === undefined || n === null) return '-';
                    const abs = Math.abs(n).toFixed(2);
                    return (n >= 0 ? '+' : '-') + abs;
                  };
                  const fmtDate = (ts) => ts ? new Date(ts).toLocaleString('en-GB', { timeZone: 'Asia/Karachi', day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '-';

                  const categoryMeta = {
                    deposit:          { label: 'Deposit',           color: '#10b981', icon: '⬇️' },
                    withdrawal:       { label: 'Withdrawal',        color: '#ef4444', icon: '⬆️' },
                    withdrawal_lock:  { label: 'Withdrawal Held',   color: '#f59e0b', icon: '🔒' },
                    transfer:         { label: 'Transfer',          color: '#6366f1', icon: '🔄' },
                    reward:           { label: 'Reward',            color: '#10b981', icon: '🎁' },
                    level_reward:     { label: 'Level Reward',      color: '#10b981', icon: '🏆' },
                    penalty:          { label: 'Penalty',           color: '#ef4444', icon: '⚠️' },
                    signal:           { label: 'Signal',            color: '#6366f1', icon: '📊' },
                    referral_signal:  { label: 'Referral Signal',   color: '#8b5cf6', icon: '🎯' },
                  };
                  const meta = categoryMeta[tx.category] || { label: tx.category, color: theme.subtext, icon: '•' };

                  // Status badge
                  const statusColor = {
                    completed: '#10b981', done: '#10b981', won: '#10b981',
                    pending: '#f59e0b', open: '#6366f1',
                    rejected: '#ef4444', lost: '#ef4444', cancelled: '#9ca3af', timed_out: '#f59e0b',
                  }[tx.status] || theme.faint;

                  const amountColor = tx.amount > 0 ? '#10b981' : tx.amount < 0 ? '#ef4444' : theme.subtext;

                  return (
                    <div key={tx.id || i} style={{ padding: '12px 0', borderBottom: `1px solid ${theme.cardBorder}` }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px' }}>
                            <span style={{ fontSize: '14px' }}>{meta.icon}</span>
                            <span style={{ fontWeight: '600', fontSize: '13px', color: meta.color }}>{meta.label}</span>
                            {tx.wallet && <span style={{ fontSize: '10px', color: theme.faint, background: theme.inputBg, padding: '1px 6px', borderRadius: '4px' }}>{tx.wallet.toUpperCase()}</span>}
                            <span style={{ fontSize: '10px', fontWeight: '600', color: statusColor, background: theme.inputBg, padding: '1px 6px', borderRadius: '4px', textTransform: 'uppercase' }}>{(tx.status || '').replace('_', ' ')}</span>
                          </div>
                          <div style={{ fontSize: '11px', color: theme.subtext, marginBottom: '2px' }}>{tx.description}</div>
                          <div style={{ fontSize: '10px', color: theme.faint }}>
                            {fmtDate(tx.at)}
                            {tx.settleAt && tx.settleAt !== tx.at && <span> · Settles: {fmtDate(tx.settleAt)}</span>}
                            {tx.network && <span> · {tx.network.toUpperCase()}</span>}
                            {tx.txHash && <span> · <span style={{ fontFamily: 'monospace', fontSize: '9px' }}>{tx.txHash.slice(0,12)}…</span></span>}
                          </div>
                          {/* Signal extra: direction, pair, prices */}
                          {(tx.category === 'signal' || tx.category === 'referral_signal') && (
                            <div style={{ fontSize: '10px', color: theme.faint, marginTop: '2px' }}>
                              {tx.pair} · {tx.direction?.toUpperCase()}
                              {tx.entryPrice ? ` · Entry: ${Number(tx.entryPrice).toLocaleString()}` : ''}
                              {tx.closePrice ? ` → Close: ${Number(tx.closePrice).toLocaleString()}` : ''}
                            </div>
                          )}
                          {/* Withdrawal extra: wallet address */}
                          {tx.category === 'withdrawal' && tx.walletAddress && (
                            <div style={{ fontSize: '9px', color: theme.faint, fontFamily: 'monospace', marginTop: '2px' }}>{tx.walletAddress}</div>
                          )}
                        </div>
                        <div style={{ textAlign: 'right', minWidth: '80px' }}>
                          <div style={{ fontWeight: 'bold', fontSize: '14px', color: amountColor }}>{fmtAmt(tx.amount)}</div>
                          {tx.profit !== undefined && tx.profit !== 0 && (
                            <div style={{ fontSize: '11px', color: tx.profit > 0 ? '#10b981' : '#ef4444' }}>
                              P&L: {tx.profit > 0 ? '+' : ''}${tx.profit?.toFixed(2)}
                            </div>
                          )}
                          {tx.netPayout !== undefined && tx.category === 'withdrawal' && (
                            <div style={{ fontSize: '10px', color: theme.faint }}>Net: ${tx.netPayout?.toFixed(2)}</div>
                          )}
                        </div>
                      </div>
                    </div>
